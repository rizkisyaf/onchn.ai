...
import { Search as SearchIcon, Loader2, Brain as BrainIcon } from 'lucide-react'
import { ErrorBoundary } from '@/components/error-boundary'
import { useSubscription } from '@/hooks/useSubscription'

export default function BehaviorAnalysisPage() {
  const { canAccessFeature, currentPlan } = useSubscription()
  const canAccessBehaviorAnalysis = canAccessFeature('advancedAnalytics')

  if (!canAccessBehaviorAnalysis) {
    return (
      <div className="p-6">
        <Card className="p-6 bg-white/5 border-white/10">
          <h2 className="text-xl font-semibold mb-4">Feature Restricted</h2>
          <p className="text-gray-400 mb-4">
            Behavior analysis is only available for paid plans. Upgrade your subscription to access this feature.
          </p>
  ...
  return (
    <ErrorBoundary>
      <div className="space-y-8">
        ...
        >
          <BehaviorAnalysis address={address} />
        </motion.div>
      </div>
    </ErrorBoundary>
  )
